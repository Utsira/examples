...
function setup() 
    ...
    toonShader=shader(FrameBlendNoTexToon.splineVert, FrameBlendNoTexToon.frag)
    parameter.watch("FPS")
...
function LoadModel() 
    local m=1
    model=Rig(Models[m].name,Models[m].mtl,Models[m].actions)
    oldDraw=draw
    draw=loadDraw
end
...
function Rig:BuildMesh() --concatenate files into mesh
    ...
    m.shader=toonShader --splineShader -- linearShader -- 
...
FrameBlendNoTexToon = { --2-pass Toon shader inspired by Unity
    ...
    {       
       ...
       vec3 frameNorm = CatmullRom(frameBlend, frameBlend2, frameBlend3, getNorm(frames[0]), getNorm(frames[1]), getNorm(frames[2]), getNorm(frames[3]) );
        ...
        {
            vec4 norm = normalize(modelMatrix * vec4( frameNorm, 0.0 ));
            vIntensity = max( 0.0, dot( norm, light )); 
            vAmbient = color * ambient;
            vAmbient.a = 1.; 
            vColor = color; 
        }
